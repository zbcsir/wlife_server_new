...
if (isset($_POST['orderstr']) && isset($_POST['gate'])) {
    ...
    if($stmt->rowCount() > 0){
        ...
        for($i=0 ;$i<$count ;$i++){
            $equal = $res[$i][1] ;
            $name = $res[$i][0] ;
            $sql_n = "SELECT `actions` FROM `orders` WHERE `name`='{$name}' AND `gate`='{$gate}'" ;
            if(($equal == 1 && $orderstr == $name) || ($equal == 0 && strstr($orderstr,$name))){
                $stmt = $pdo->query($sql_n) ;
                $action = $stmt->fetch() ;
                $order = $action[0] ;
                $is_find = true ;
            }break ;
    ...
    $mode_obj = array('窗户','窗帘','空调','客厅灯','卧室灯','客厅的灯','卧室的灯','灯');
